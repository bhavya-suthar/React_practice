...
const FileInput = ({showModal, setShowModal, handleClose, handleShow,onImageSelected}) => {
  const onChooseImg =()=>{
        ...
        <div className="d-flex justify-content-center m-5">
          <button className="bg-white p-4 border-0 position-relative">
            ...
            <label htmlFor="" className="spanCamera" onClick={onChooseImg}>
              <span>
                <TbCameraPlus style={{ height: "45px", width: "45px",padding:"5px" }} />
              </span>
            </label>
          </button>

      <input type='file' name='addpicture'  id="addpicture" accept="image/*" className="d-none" onChange={handleOnImgChange}/>
        </div>
      </div>
    </>
  );
